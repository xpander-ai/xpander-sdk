...
XPANDER_AGENT_ID_WITH_IO_SCHEMAS = os.getenv("XPANDER_AGENT_ID_WITH_IO_SCHEMAS")
...
@pytest.mark.asyncio
async def test_agent_with_schemas():
    agents = Agents()
    agent = await agents.aget(agent_id=XPANDER_AGENT_ID_WITH_IO_SCHEMAS)

    assert isinstance(agent, Agent)
    assert agent.id == XPANDER_AGENT_ID_WITH_IO_SCHEMAS
    assert len(agent.tools.list) != 0
    
    await agent.acreate_task("search for hello world in tavily! fake the params if needed, just call the tool until you make it")
